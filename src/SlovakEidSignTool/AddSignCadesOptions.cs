...
namespace SlovakEidSignTool
{
    ...
    public class AddSignCadesOptions
    {
        ...
        public static IEnumerable<Example> Examples
        {
            get
            {
                yield return new Example("Sign text document", new AddSignCadesOptions() { ContainerFile= "signedDocument.asice", SourceFile = "hello.pdf", SourceFileMimeType = "plain/text", DestinationFile = "hello.asice", LibPath = null, UseEidClientPin = false });
                yield return new Example("Sign PNG image with eID client for PIN typing", new AddSignCadesOptions() { ContainerFile = "signedDocument.asice", SourceFile = "hello.png", SourceFileMimeType = "image/png", DestinationFile = "hello.asice", UseEidClientPin = true });
                yield return new Example("Sign PDF with specific PKCS11 lib", new AddSignCadesOptions() { ContainerFile = "signedDocument.asice", SourceFile = "document.pdf", SourceFileMimeType = "application/pdf", DestinationFile = "document.asice", LibPath = "/opt/SkEid/bin/pkcs11_x64.so", UseEidClientPin = false });
            }
        }
    }
}
