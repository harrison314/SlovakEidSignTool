...
{
    internal static class Session41Extension
    {
        public static byte[] SignWithPin(this Session session, Mechanism mechanism, ObjectHandle keyHandle, byte[] data, byte[] pin)
        {
            ...
            Net.Pkcs11Interop.LowLevelAPI41.Pkcs11 _p11 = session.LowLevelPkcs11;
        ...
        public static byte[] SignWithPin(this Session session, Mechanism mechanism, ObjectHandle keyHandle, byte[] data, SecureString pin)
        {
            CK_MECHANISM ckMechanism = (CK_MECHANISM)typeof(Mechanism).GetField("_ckMechanism", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(mechanism);
            Net.Pkcs11Interop.LowLevelAPI41.Pkcs11 _p11 = session.LowLevelPkcs11;
            CKR rv = _p11.C_SignInit(session.SessionId, ref ckMechanism, keyHandle.ObjectId);
            ...
            if (pin == null)
            {
                rv = _p11.C_Login(session.SessionId, CKU.CKU_CONTEXT_SPECIFIC, null, 0);
            }
            else
            {
                SecurityUtils.ExecuteUsingSecureUtf8String(pin, pinBuffer =>
                {
                    rv = _p11.C_Login(session.SessionId, CKU.CKU_CONTEXT_SPECIFIC, pinBuffer, Convert.ToUInt32(pinBuffer.Length));
                });
            }
