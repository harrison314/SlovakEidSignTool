...
namespace SlovakEidSignTool.LowLevelExtensions
{
    internal static class Session81Extension
    {
        ...
        public static byte[] SignWithPin(this Session session, Mechanism mechanism, ObjectHandle keyHandle, byte[] data, SecureString pin)
        {
            ...
            if (pin == null)
            {
                rv = _p11.C_Login(session.SessionId, CKU.CKU_CONTEXT_SPECIFIC, null, 0UL);
            ...
            else
            {
                ...
                {
                    rv = _p11.C_Login(session.SessionId, CKU.CKU_CONTEXT_SPECIFIC, pinBuffer, Convert.ToUInt64(pinBuffer.Length));
